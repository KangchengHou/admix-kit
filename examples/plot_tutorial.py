...
# %%
# These are `xarray.Dataset` object, which is multi-dimensional annotated
# dataset. In our application, they will have 3 dimensions. `indiv`: the
# dimension of different individual; `snp`: the dimension of SNP; `haploid`:
# the dimension of ploidy, which will be 2.
# These `xarray.Dataset` objects will be the central data structure we operate
# on.
...
# %%
fig, ax = plt.subplots(nrows=2, figsize=(4, 2), dpi=150, sharex=True)
